...
首页商品列表下方支持 `中文 / English` 切换，用户选择会保存到数据库，后续商品详情、购买、支付和常见提示会按该语言展示。

- 首页默认文案可在 `config.json` 的 `START.title`、`START.intro`、`START.title_en`、`START.intro_en` 中配置；后台主页设置会写入数据库并优先于配置文件。
- 商品英文名称/详情、档位英文名称可在管理后台的商品页和档位页维护。
- 未配置英文商品内容时会自动回退到现有中文商品名称、详情和档位名称。
